...
final class PharBuilder
{
    ...
    public static function roots(string $appDir, string $context, array $imports): array
    {
        ...
        foreach ($imports as $import) {
            ...
            $declared = AppDirs::tmpDirFor($import->appName, $import->context, $importDir, $import->writeDir);
            if ($record->tmpDir !== $declared) {
    ...
    private static function writesOutside(array $archiveBases, string $appDir, string $context): CompileRecord
    {
        ...
        $tmpDirs = [$record->tmpDir, self::resolve($record->tmpDir)];
    ...
    private static function writeDirOf(CompileRecord $record): string|null
    {
        ...
        if (! str_ends_with($record->tmpDir, $suffix)) {
            return null;
        }
    ...
    {
        $filter = static function (mixed $file) use ($appDir, $roots, $output): bool {
            ...
            if ($file->isDir() && $file->isLink()) {
                throw new PharSymlinkedDirectoryException($path);
            }

            if ($path === $output || $path === $appDir . '/tests' || $path === $appDir . '/autoload.php' || $path === $appDir . '/preload.php') {
                return false;
            }

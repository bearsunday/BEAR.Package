 ...
 * every process start, so an archive that exists belongs to this run. Under
 * --process-isolation each child process simply builds again.
 */
class PharPortabilityTest extends TestCase
{
    ...
    private static function builtArchive(): array
    {
        ...
        self::compile($paths['app'], $paths['write']);
    ...
    /** Symlinked files land as the content they point at; nothing here links a directory. */
    ...
    private static function compileScript(): string
    {
        return <<<'PHP'
            ...
            /**
             * Compile the fixture, then pack it into app.phar.
             *
             * Usage:  APP_WRITE_DIR=... php bin/compile.php
             */

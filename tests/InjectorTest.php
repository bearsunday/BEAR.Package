...
class InjectorTest extends TestCase
{
    ...
    /** Two trees of one application can be booted in one process, and the second must not be handed the first one's paths. */
    ...
    /** An override injector writes where the default one does: the declaration is the application's, not the caller's. */
    public function testGetOverrideInstanceWritesWhereTheApplicationDeclared(): void
    {
        $context = 'readonly-app';
        $default = Injector::getInstance('FakeVendor\HelloWorld', $context, __DIR__ . '/Fake/fake-app');
        $override = $this->getInjector(new class implements AppInterface {
        }, $context);

        $this->assertSame(ReadonlyModule::tmpDir(), $override->getInstance(AbstractAppMeta::class)->tmpDir);
        $this->assertSame(
    ...
    /** @param non-empty-string $context */
    private function getInjector(AppInterface $fakeApp, string $context = 'app'): InjectorInterface
    {

...
//        (new Unlink)->force(__DIR__ . '/script/var');
        App::$construct = 0;
        $cmd = sprintf('php %s/script/script-app.php', __DIR__);
        $exitCode = (new AsyncRun)([$cmd], __DIR__ . 'error.log');
        $this->assertSame(0, $exitCode);

        $injector = new ScriptInjector('FakeVendor\HelloWorld', 'app', __DIR__ . '/Fake/fake-app');
        /** @var App $app */
    ...
    {
        App::$construct = 0;
        $cmd = sprintf('php %s/script/script-prod.php', __DIR__);
        $exitCode = (new AsyncRun)([$cmd], __DIR__ . 'error.log');
        $this->assertSame(0, $exitCode);

        $injector = new ScriptInjector('FakeVendor\HelloWorld', 'prod-app', __DIR__ . '/Fake/fake-app');
        /** @var App $app */
        $app = $injector->getInstance(AppInterface::class);
        $this->assertInstanceOf(AppInterface::class, $app);

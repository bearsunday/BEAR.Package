...
- `ReadOnlyAppModule($tmpDir, $logDir)`, installed from the application's own `ProdModule`, says where the application writes when its tree is read-only; a named path is compiled in as given, and whether it can be written to is answered where something writes

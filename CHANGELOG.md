...
### Added
- `bin/bear.compile` returns, deprecated, printing where to go: removing an installed command in a minor release broke `composer.json` scripts that call `vendor/bin/bear.compile`, so it stays until 2.0 (#482)
